...
#========================================================
#[X Y Z]
def score_cal(rotation_vector, position):
    if position == 'front':
        scoreX = 100-((abs(rotation_vector[0]+3.142)/1.57)*2.5)*100
        scoreZ = 100-(abs((abs(rotation_vector[2])-0.15))/1.57)*2*100

        # print(scoreX)
        # print(scoreZ)

        score = (0.7*scoreZ + 0.3*scoreX)*1.2
        #print(score > 0)
        ...
        while True:
            ...
            if results.multi_face_landmarks:                                            # if faces found
                ...
                for faceNum, faceLms in enumerate(results.multi_face_landmarks):                            # loop through all matches
                    ...
                    rotM = cv2.Rodrigues(rotation_vector)
                    
                    #print("translation:\n",translation_vector)
                    print("rotation V:\n",rotation_vector)
                    #print("rotation M:\n",rotM)
                    score = score_cal(rotation_vector,'front')
